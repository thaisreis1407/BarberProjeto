  ...
  private configuraAcoes() {
    this.acaoItens =  [
...

      {
        label: 'Alterar',
        icon: 'pi pi-pencil',
        disabled: !this.auth.temPermissao('ROLE_ALTERAR_DUPLICAT_APAGAR'),
        command: () => {
      ...
      },
      {
        label: 'Excluir',
        icon: 'pi pi-trash',
        disabled: !this.auth.temPermissao('ROLE_EXCLUIR_DUPLICAT_APAGAR'),
        command: () => {
          if (this.idDuplicataPagarSelecionado === 1) {
            this.msgService.erro('Duplicata não pode ser excluída');
          } else {
