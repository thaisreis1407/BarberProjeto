...
export class ServicoService  extends BaseService<Servico> {
  ...
  constructor(private customHttp: CustomHttp,
    ...
    private utilService: UtilService) {
      ...
      this.serviceURL = configService.apiUrl + '/servicos';
      this.servicoFiltro = new ServicoFiltro(configService);
    }

    getFiltro(): ServicoFiltro {
      // para gerar instancia unica juntamente com o servico
      return this.servicoFiltro;
    }
    ...
    buscarPorId(id: number): Promise<Servico> {
      ...
      return this.http.get<Servico>(`${this.serviceURL}/${id}`, { headers })
        .toPromise()
        .then(response => {
  ...
  consultar(filtro: ServicoFiltro): Promise<any> {
    const headers = new HttpHeaders();
